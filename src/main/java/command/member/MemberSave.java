...
public class MemberSave implements CommonExecute {
	...
	public void execute(HttpServletRequest request) {
		...
		// 소문자 최소 1자 포함, 4~20자
		// 영문 대/소문자, 숫자, !@#$%^&*_-. 허용
		String idPattern = "^(?=.*[a-z])[a-zA-Z0-9!@#$%^&*_.-]{4,20}$";

		if (isEmpty(id) || !id.matches(idPattern)) {
			alert(request, "영문 소문자를 최소 1자 이상 포함하여 4~20자로 입력해주세요. " + "특수문자는 !@#$%^&*_-.만 가능합니다.");
			return;
		}
		...
		if (isEmpty(password) || !password.matches(passwordPattern)) {
			alert(request, "비밀번호는 영문 소문자를 최소 1자 이상 포함해 6~16자로 입력해주세요. " + "특수문자는 !@#$%^&*_+?.-만 사용할 수 있습니다.");
			return;
		}
		...
		if (verifiedEmail == null || !verifiedEmail.equals(email.trim())) {
			alert(request, "이메일 인증을 완료해주세요.");
			return;
		}
